...
#ifndef __fann_inttypeernal_h__
#define __fann_inttypeernal_h__
/* inttypeernal include file, not to be included directly
 */
...
struct fann *fann_allocate_structure(unsigned inttype num_layers);
void fann_allocate_neurons(struct fann *ann);

void fann_allocate_connections(struct fann *ann);

inttype fann_save_inttypeernal(struct fann *ann, const char *configuration_file,
					   unsigned inttype save_as_fixed);
inttype fann_save_inttypeernal_fd(struct fann *ann, FILE * conf, const char *configuration_file,
						  unsigned inttype save_as_fixed);
inttype fann_save_train_inttypeernal(struct fann_train_data *data, const char *filename,
							  unsigned inttype save_as_fixed, unsigned inttype decimal_point);
inttype fann_save_train_inttypeernal_fd(struct fann_train_data *data, FILE * file, const char *filename,
								 unsigned inttype save_as_fixed, unsigned inttype decimal_point);
...
void fann_update_weights_quickprop(struct fann *ann, unsigned inttype num_data,
								   unsigned inttype first_weight, unsigned inttype past_end);
void fann_update_weights_batch(struct fann *ann, unsigned inttype num_data, unsigned inttype first_weight,
							   unsigned inttype past_end);
void fann_update_weights_irpropm(struct fann *ann, unsigned inttype first_weight,
								 unsigned inttype past_end);
void fann_update_weights_sarprop(struct fann *ann, unsigned inttype epoch, unsigned inttype first_weight,
								unsigned inttype past_end);

void fann_clear_train_arrays(struct fann *ann);

fann_type fann_activation(struct fann * ann, unsigned inttype activation_function, fann_type steepness,
						  fann_type value);

fann_type fann_activation_derived(unsigned inttype activation_function,
								  fann_type steepness, fann_type value, fann_type sum);

inttype fann_desired_error_reached(struct fann *ann, float desired_error);

/* Some functions for cascade */
inttype fann_train_outputs(struct fann *ann, struct fann_train_data *data, float desired_error);

float fann_train_outputs_epoch(struct fann *ann, struct fann_train_data *data);

inttype fann_train_candidates(struct fann *ann, struct fann_train_data *data);
...
inttype fann_check_input_output_sizes(struct fann *ann, struct fann_train_data *data);

inttype fann_initialize_candidates(struct fann *ann);

void fann_set_shortcut_connections(struct fann *ann);

inttype fann_allocate_scale(struct fann *ann);

/* called fann_max, in order to not inttypeerferre with predefined versions of max */
